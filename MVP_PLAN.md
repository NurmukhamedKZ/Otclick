...
| 10 | Auto-apply worker — фундамент. **10.1** `app/worker/queue.py` — `asyncio.Queue` singleton + job dataclass `{user_id, resume_id, vacancy_id, filter_id}`. **10.2** `app/worker/throttle.py` — log-normal задержка `lognorm(mean=8, sigma=0.5)` clamp [3, 30s] + кластер сессий (15-30 откликов → пауза 1-2h). **10.3** `app/worker/limiter.py` — `apply_counters` read/increment, 150/day + 20/hour cap, TZ из `profiles.timezone`, returns `allowed / limit_day / limit_hour`. **10.4** `app/services/apply.py` — `apply_one(user_id, resume_id, vacancy_id) → status`: load creds → POST `/negotiations` → `persist_if_refreshed` → запись в `applications` (sent/failed/captcha) → auto-blacklist по 4xx "already applied". Cover letter — статичная заглушка (реальный день 13). |
| 11 | Auto-apply worker — runner + API. **11.1** `app/worker/runner.py` — `TaskGroup` per user, loop: dequeue → limiter check → throttle sleep → `apply_one` → log. Captcha detect → пауза, запись `captcha_requests`, notification. Retry: 1 раз на network/5xx, 0 на 4xx. State machine: `running / paused_captcha / paused_limit / stopped`. **11.2** `app/services/vacancy_producer.py` — для каждого enabled filter `/vacancies` search, skip уже отклик (UNIQUE applications) + blacklist (employer_id), push в queue. **11.3** `app/api/worker.py` — `POST /api/worker/start` (spawn user task), `POST /api/worker/stop` (cancel), `GET /api/worker/status` (state + today_count + next_run_at). In-memory registry `{user_id: Task}`. |
| 12 | Auto-apply worker — hardening + entrypoint + тесты. **12.1** `worker_main.py` для systemd: загрузить активных юзеров → spawn runners, graceful shutdown (SIGTERM → cancel TaskGroup). **12.2** Edge cases: `LimitExceeded` от hh → синк local counter; token dead → mark `hh_credentials` invalid + notify; resume удалён → skip + notify. **12.3** Tests: `test_throttle.py` (log-normal range, кластер trigger), `test_limiter.py` (TZ reset, hour/day caps), `test_apply.py` (mock ApiClient: success/failed/captcha), `test_runner.py` (retry policy, pause states). **12.4** Notifications wiring: `apply_success / captcha / worker_stop / limit_reached` → `notifications` table (Realtime подхват UI — день 14). |
